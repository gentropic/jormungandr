...
- Watchdog: every guest resume passes through supervisor code (all awaits go through `hal`), so the supervisor stamps the resuming guest's id into a **current-guest register** on entry and clears it when control returns to the loop. A heartbeat task timestamps each loop pass; on starvation > 250 ms, the guest named in the register is flagged `unresponsive` — attribution by evidence of *who holds the CPU*, not who yielded longest ago. (A guest parked on a long `await` has a very stale yield time and is innocent by construction; staleness heuristics blame long-sleepers, not spinners.)
- **The worst case, said out loud** (mpy): a guest that *never* yields starves the whole loop — the heartbeat, the flagging logic, **and the web server**. Exactly when you'd want the UI to show "unresponsive," it's unreachable. So the supervisor arms the **hardware WDT**, and the current-guest register lives in **RTC memory** (`machine.RTC().memory()` — survives a WDT reset, costs a RAM write per resume, no flash wear). After a reset, an *uncleared* stamp names the guest that held the CPU when the watchdog fired; its `autostart` is **disabled** and the guest badged: *"suspected in watchdog reset — autostart disabled."* An empty register after reset means the stall was the supervisor's own — say so in the node log rather than framing a guest. Crash-loop protection with a named culprit; the node always comes back reachable.
...
`/api/lib` semantics: `PUT` is last-write-wins on the store; replacing a library currently imported by a **running** guest is refused without `?force` (the guest keeps its loaded copy until restart regardless — Python's import cache sees to that; `?force` is for people who know this). The import graph behind both refusals comes from a **static scan of bundle sources at install time** — cheap, and its one blind spot is stated: there is no dynamic import in the guest SDK, so there is nothing for the scan to miss.

Transport is plain HTTP + bearer token; the trust boundary is the LAN (or the tailnet, if the node must be reachable beyond it). TLS on-MCU buys little here and costs RAM plus certificate liturgy — stated as a decision, not an oversight. One composition deserves its own sentence: a node holding a `hid` grant is a keystroke injector into a host machine, and a sniffed bearer token on that node types on that host — **HID-bearing nodes are the ones that most deserve the tailnet posture**, and the UI's keystroke-injector flag (§3) doubles as the reminder.
...
- **Grant-checked at declaration.** Every `set` must fall within the declaring guest's own `sub` grants — a panel can command its guest, never its neighbors; violations refuse the declaration. `bind` is checked *softly*: a bind no current claimant may publish renders as a stale widget instead of refusing, so install order between a publisher and a panel-declarer never matters (the renderer already speaks stale, §"panels outlive their guests").
...
Zero also fixes, by fiat rather than agony: MicroPython proper ≥ 1.24 for the mpy profile (CircuitPython is a *driver-API reference* for sol's `/lib`, not a target runtime); provisioning via `settings.json` at flash time; nodes announce as `jorm-<mac4>` over mDNS; supervisor ships as plain `.py` files on the board FS (freeze-to-firmware later); the `jorm` CLI is born alongside milestone 1 as API client *and* integration test harness. Development board for zero: an **ESP32-S3** — M1 fits a classic WROOM-32, but M2's WebSocket load (bus bridge + console streams + broker queues, all live at once) is where classic-ESP32 heap gets famously fragile, and the S3 also unlocks §8. A WROOM-32 remains a valid M1 fallback; the C3 leaves keep the sol profile honest either way.

### Revisions (2026-07-12)

10. **Watchdog attribution rebuilt** (§1): the stalest-`last_yield` heuristic blamed long-sleepers, not spinners, and the flash blame-file couldn't be written during a hard stall (the heartbeat never runs again). Replaced with a current-guest register stamped on every resume and mirrored in RTC memory — survives the WDT reset, names whoever actually held the CPU.
11. **Panel `bind` check softened** (§7): dangling binds render stale instead of refusing, removing the install-order dependency between publishers and panel-declarers. `set` stays hard-checked.
...
18. **Time is monotonic inside, Unix at the boundary** (§4). Two traps, both of which produce *plausible* wrong answers: MicroPython's epoch is 2000-01-01 (every consumer of our JSON uses 1970), and an MCU boots with no clock at all — so a wall-clock stamp taken before NTP lands is uptime wearing a costume, and it renders as a perfectly reasonable time in a console pane. The node therefore records **monotonic** time and converts at the API boundary: `ts` is Unix seconds, or **null** while the clock is unset (the UI shows uptime and badges the node ⚠ clock unsynced). When NTP lands, the offset is learned once and applied to everything — *including the past*, so lines written before the sync are dated correctly rather than left as fiction. NTP at boot with backoff, then daily; the sim reports `source: host` because its OS clock is already right, and the flag must not lie in either direction.
19. **Supervisor OTA** (§6/§11): the supervisor is plain `.py` on a filesystem behind an authenticated API, so it can update itself. Files are staged over HTTP (`PUT /api/node/files/<path>`), `POST /api/node/update` reboots, and `boot.py` — which imports nothing from `jorm`, because a broken `jorm/` is exactly what it recovers from — applies the staged tree over a backup and marks the boot a **trial**. A trial confirms itself by *being a healthy node*: main.py clears the marker once the API has served for 20 s. If it never gets there — crash, hang, watchdog — the next boot finds the marker still set, and an unconfirmed trial is a failed one: restore the backup, boot the old supervisor, badge the node. A bad push costs a reboot, not a cable. This is §1's promise (*the node always comes back reachable*) extended from guests to the supervisor itself. Firmware OTA is a separate thing and is **not** available on a stock partition table — one `factory` app slot, no A/B pair.
...
27. **A board is a name** (§11). `jorm nodes add c510 http://jorm-c510.local --token …` writes a registry (`%APPDATA%/jorm/nodes.json`, or `~/.config/jorm/`, chmod 600 — it holds bearer tokens), and every command takes `-n/--node`. Without it a second node is *technically* supported and *practically* never used, which is the same as not supporting it: nobody retypes a 32-character token to check on a board. Precedence: an explicit `--url` always wins, so a script can override a person's saved default. Discovery is still v1 — the registry is how you name what you already know about, not how you find it.
28. **`console -a` attaches** (§4/§11). The node has taken console input since `hal.console.input()` landed, but the CLI could only ever watch a guest talk. **A console you can only read is a log** — the thing the UI already refused — so the CLI refuses it too: `jorm console parrot -a` streams the guest and types into it.
29. **No elected front door — every node is one** (§9). MicroPython's ESP32 port starts the IDF mDNS responder at network init and holds UDP 5353 exclusively; binding it from Python is `EADDRINUSE` before connect, after connect, with `SO_REUSEADDR`, and even if `network.hostname()` is never called. There is no `mdns` module. **One name per node, and it is not negotiable from Python** — so a cluster cannot elect a node to also answer `jormungandr.local` without a firmware fork. It does not need to: **Proxmox has no cluster VIP either.** You browse to any node and manage the whole cluster from it. Every node is the front door, which is strictly better than an elected one — no election, no flapping, no split brain, no single point of failure. (ESP-IDF *does* expose `mdns_delegate_hostname_add()`; revisit if there is ever a firmware project. Not before.)
30. **Signal strength is a placement input, not a routing one** (§9). A node reports `rssi`, or `null` if it has no radio — *not* 0 dBm, which would make a wired sim the best-connected node in the cluster and a placement score would act on it. The question "least load, best link" is the right question; its answer belongs to *where a migrated guest should land*, not to DNS.
31. **`jorm open` probes and launches** (§11). It ranks by round-trip time from *your* machine — what a browser actually feels — and breaks near-ties on RSSI, since a weak link is a slow node that has not been slow yet.
32. **A browser is handed a ticket, never a token** (§2). `POST /api/auth/ticket` mints 128 bits of urandom, good for one redemption and sixty seconds; `POST /api/auth/redeem` is the single endpoint the token cannot guard, because redeeming is how you get one. The UI reads it from the URL *fragment* and strikes it from history before doing anything else. A fragment never reaches the server — but it does reach history, autocomplete, and "copy link", so what goes there must be worthless a minute later. **The bearer token only ever travels in a header.**
