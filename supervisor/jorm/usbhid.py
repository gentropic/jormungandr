# Vendored from micropython-lib: micropython/usb/usb-device-hid/usb/device/hid.py
# (MIT, (c) 2023-2024 Angus Gratton / MicroPython contributors)
#
# Why vendored and not installed via mip: this is supervisor code, not a guest
# library. A guest reaches USB through hal.usb and never imports it, exactly as it
# never imports machine or network — the interface IS the isolation. Putting it in
# /lib would offer it to guests, and a guest that can build its own USB descriptor
# owns the whole device, not the one interface it was granted.
#
# Changed from upstream: the package-relative imports, because jorm's flash is flat.
...
import struct
import time
from jorm.usbcore import Interface, Descriptor, split_bmRequestType
