# Vendored from micropython-lib (MIT, (c) 2023-2024 Angus Gratton / MicroPython
# contributors). Supervisor code, not a guest library — see jorm/usbcore.py.
# Changed from upstream: package-relative imports, because jorm's flash is flat.
...
class KeyboardInterface(HIDInterface):
    ...
    def __init__(self, interface_str="jorm keyboard"):
        # interface_str added over upstream: the host's device list shows one entry
        # per interface, and "jorm: typist" is worth more than three identical
        # "MicroPython Keyboard" rows when several guests own several interfaces.
        super().__init__(
